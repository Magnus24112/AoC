...
def comparehands(hand1: list, hand1strength: int, hand2: list, hand2strength: int) -> bool:
    '''
    True if hand1 is stronger than hand 2
    False is hand2 is stronger (or the same) than hand 1
    '''
...
def quicksort(unsorted: list) -> list:
    '''
    unsorted[0] looks like: [[card1, card2, ..., card5], strength, bet]
    '''
...
def best_strength(hand: list) -> int:
    if not 'J' in hand:
        ...
        uniquecards = []
        for card in hand:
            if card != 'J' and card not in uniquecards:
                uniquecards.append(card)
        bestrecordedstrength = 0
        for uniquecard in uniquecards:
            tmphand = []
            for card in hand:
                tmphand.append(uniquecard if card == 'J' else card)
            bestrecordedstrength = max(bestrecordedstrength, handstrength(tmphand))
        return bestrecordedstrength
